...
基于 v1 trainer，改动:
- 使用 SymFoldModel_v3 (DA-SE-DiT + Physics-Aware Loss)
- Loss dict 包含 bce/stack/nc 分项
- 支持 v3 特有配置 (dilation_pattern, stack_weight, nc_weight)
...
def setup_logging(config):
    ...
    return logging.getLogger('SymFold-v3')
        ...
        try:
            ...
            if bi % log_every == 0:
                logger.info(
                    ...
                    f'nc={ld.get("nc", 0):.5f} t={time.time()-t0:.3f}s')
...
def main():
    cfg_path = os.path.join(TRAIN_DIR, 'config', 'train_config_v3.json')
    if len(sys.argv) > 1:
        cfg_path = sys.argv[1]
    ...
    logger.info('=' * 70)
    logger.info(f'SymFold v3 Training: {task}')
    ...
    logger.info('[Init] building v3 model on CPU...')
    t0 = time.time()
    mc = config['model']
    ...
    if config['training'].get('auto_resume', True) and os.path.isfile(last_ckpt):
        try:
            ...
            # 如果 checkpoint 中 history 为空，从磁盘 history.json 恢复
            ...
            if not history and os.path.isfile(history_json):
                try:
                    ...
                    logger.info(f'[Resume] recovered history from {history_json} ({len(history)} entries)')
                except Exception as he:
                    logger.warning(f'[Resume] failed to load history.json: {he}')

            # 从 history 中重算 best_f1（比 checkpoint 中的更可靠）
